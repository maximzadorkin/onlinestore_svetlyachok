...
USE `OnlineStore`$$
DROP TRIGGER IF EXISTS `OnlineStore`.`Сделки_AFTER_DELETE` $$
USE `OnlineStore`$$
CREATE DEFINER = CURRENT_USER TRIGGER `OnlineStore`.`Сделки_AFTER_DELETE` AFTER DELETE ON `Сделки` FOR EACH ROW
BEGIN
